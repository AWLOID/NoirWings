...
namespace IronBrew2
{
	...
	{
		public static Random Random = RandomProvider.Create();
		private static Encoding _fuckingLua = Encoding.Latin1;
		...
		public static bool Obfuscate(string path, string input, ObfuscationSettings settings, out string error)
		{
			try
			{
				...
				File.WriteAllText(t1, new ConstantEncryption(settings, File.ReadAllText(t0, _fuckingLua)).EncryptStrings(), _fuckingLua);
				proc = new Process
				       {
					       StartInfo =
					       {
